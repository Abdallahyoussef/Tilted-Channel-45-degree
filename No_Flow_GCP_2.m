...
% The channel is the same as CASE E except that the channel is tilted by
% angle theta and constant pressure boundaries at left and right boundaries
...
for i=1:lx;
    if i==1
        ...
        Kx(2*(i-1):2*(i-1)+length(Kx_cave)-1,i)=Kx_cave;
    end
end
...
Kx=Kx';Kx=Kx(:);
Ky=Kx;
Theta=pi/4; % inclination angel of channel by radian
...
startx=[(dx/2)*ones(1,ly/5) 0.5*dx:dx:dx*(lx-0.5)]; starty=[dy/2:5*dy:dy*(ly-0.5) linspace(dy/2,15,lx)]; hlines=streamline(x,y,vec2mat(Fx,lx),vec2mat(Fy,lx),startx,starty);
